...
public class RawMaterialStockDAO {
	...
	public List<RawMaterialStockDTO> getRawMaterialStocks(String searchKeyword, int pageNumber, int pageSize) {
		List<RawMaterialStockDTO> stockList = new ArrayList<>();
		String query = "SELECT * FROM (SELECT ROWNUM rnum, stock_id, item_code, stock_name, stock_count, stock_location, stock_sort "
				+ "FROM (SELECT * FROM stock WHERE stock_sort = '완제품' AND (item_code LIKE ? OR stock_name LIKE ?) ORDER BY item_code) "
		...
		try (Connection con = Database.getConnection(); PreparedStatement ps = con.prepareStatement(query)) {
			...
			while (rs.next()) {
				RawMaterialStockDTO stock = new RawMaterialStockDTO();
	...
	public int getTotalCount(String searchKeyword) {
		String query = "SELECT COUNT(*) FROM stock WHERE stock_sort = '완제품' AND (item_code LIKE ? OR stock_name LIKE ?)";
		int totalCount = 0;
